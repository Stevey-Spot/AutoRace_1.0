...
public class hauptklasse 
{
	...
	int zielkilometer=Math.round(anfangsbenzinwert*(20+(int)(Math.random()*4)));
	...
	/*for (int i=0;i<10;i++)
		{
			System.out.println(autos[i].geschwindigkeit);
		}
	 */
	//Autos Nummern geben
	...
	spielerauto.geschwindigkeit=89;
	spielerauto.zeit=3;
	spielerauto.zeit=spielerauto.zeit-spielerauto.geschwindigkeit/3.2;
	System.out.println(spielerauto.zeit);
		...
		for (int k=0;k<10;k++)
		{
			...
			else 
			{
				if ((zielkilometer-autos[k].kilometer)>= autos[k].geschwindigkeit)
				{
					...
					{
						autos[k].fahrenbisinsziel(zielkilometer);
					}
				}
